...
import { fetchElectricityData, fetchFuelData, fetchTravelBySpendData } from '../../../services/ClimatiqFunctions';
import { Box, FormControl, FormControlLabel, Radio, RadioGroup, Tab, Tabs } from '@mui/material';
import { CustomEmployeeIcon, ElectricityIcon, FuelIcon } from '../../../assets/icons';
...
const EmployeeCommuting: FC<Props> = ({ scope, setScope, company, setFinalScope, handleInputChange, showToast }) => {
      ...
      try {
        ...
        if (company) {
          const newScope3: any = {
            company: company?._id || '',
            scope: 3,
            type: `Employee Commuting (${getScopeType[value]})`,
  ...
  const handleCalculateSpendRoute = async () => {
    ...
    try {
      ...
      const newScope: Scope3 = {
        company: company?._id || '',
        scope: 3,
        type: `Employee Commuting (${types.TravelExp})`,
        amount: scope?.amount,
  ...
  useEffect(() => {
    if (selectedRoute) {
      const newScope: Scope3 = {
        company: company?._id || '',
        scope: 3,
        type: `Employee Commuting (${types.TravelDist})`,
        ...
        <Box sx={{ borderBottom: 1, borderColor: 'divider' }} className="mb-6">
          <Tabs value={value} onChange={handleChange} aria-label="basic tabs example">
            ...
            <Tab
              ...
              iconPosition="start"
              icon={<CustomEmployeeIcon fontSize={'small'} />}
              className="normal-case items-end text-base"
            />
          </Tabs>
        </Box>
        ...
        <TabPanel value={value} index={2}>
          <>
            <FormControl component="fieldset">
              ...
              >
                <FormControlLabel value="distance" control={<Radio />} label="Employee Commuting by distance" />
                <FormControlLabel value="expenditure" control={<Radio />} label="Employee Commuting by expenditure" />
              </RadioGroup>
            </FormControl>
