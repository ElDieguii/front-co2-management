...
const WasteOperation: FC<Props> = ({ scope, setScope, company, setFinalScope, handleInputChange, showToast }) => {
      ...
      try {
        ...
        if (company) {
          const newScope3: any = {
            company: company?._id || '',
            scope: 3,
            type: `Waste in Operations (${getScopeType[value]})`,
  ...
  useEffect(() => {
    if (selectedRoute) {
      const newScope: any = {
        company: company?._id || '',
        scope: 3,
        type: `Waste in Operations (${selectedRoute.type})`,
